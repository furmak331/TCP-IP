...
class Link:
    ...
    def is_medium_busy(self):
        ...
        return self.medium_busy or len(self.transmitting_devices) > 0
    ...
    def start_transmission(self, device):
        ...
        if self.is_medium_busy():
            self.logger.info(f"{device.name} sensed medium busy, deferring transmission")
            return False
        
        # Medium is free, start transmitting
        self.transmitting_devices.add(device)
        self.medium_busy = True
        self.logger.info(f"{device.name} started transmission, medium is now busy")
    ...
    def end_transmission(self, device):
        ...
        # Set medium to free if no devices are transmitting
        if len(self.transmitting_devices) == 0:
            self.medium_busy = False
            self.logger.info(f"Medium is now free")
    ...
    def transmit(self, frame, source):
        ...
        while attempts < CSMA_CD_MAX_ATTEMPTS:
            # Wait for any previous backoff
            if backoff_time > 0:
                self.logger.info(f"{source.name} backing off for {backoff_time:.3f}s (attempt {attempts+1})")
                time.sleep(backoff_time)
            
            # Check if medium is busy (carrier sense)
            if self.is_medium_busy():
                self.logger.info(f"{source.name} sensed medium busy, deferring transmission")
                time.sleep(TRANSMISSION_DELAY)  # Wait a bit before trying again
                continue
            
            # Start transmission
            if not self.start_transmission(source):
                # Medium became busy or collision occurred
                attempts += 1
                # Calculate backoff using exponential backoff algorithm
                k = min(attempts, 10)  # Cap at 10 to avoid excessive backoff
                r = random.randint(0, 2**k - 1)
                backoff_time = r * CSMA_CD_SLOT_TIME
                continue
            
            # Simulate transmission delay
            self.logger.info(f"Transmitting frame from {source.name} to {destination.name}")
            time.sleep(TRANSMISSION_DELAY)
            
            # Check for collision during transmission
            if self.collision_detected:
                self.logger.warning(f"Collision detected during transmission from {source.name}")
                # Send jam signal
                self.logger.info(f"{source.name} sending jam signal")
                time.sleep(TRANSMISSION_DELAY / 10)  # Short jam signal
                
                # End transmission and try again with backoff
                self.end_transmission(source)
                attempts += 1
                # Calculate backoff using exponential backoff algorithm
                k = min(attempts, 10)  # Cap at 10 to avoid excessive backoff
                r = random.randint(0, 2**k - 1)
        ...
        self.logger.error(f"{source.name} exceeded maximum transmission attempts ({CSMA_CD_MAX_ATTEMPTS})")
        self.end_transmission(source)
        return False
...
def demonstrate_csma_cd():
    ...
    # Start multiple transmissions with slight delays to create collision scenarios
    threading.Thread(target=network.send_message, args=("PC1", "Message from PC1", "PC3")).start()
    time.sleep(0.01)  # Small delay to ensure overlap
    threading.Thread(target=network.send_message, args=("PC2", "Message from PC2", "PC4")).start()
    time.sleep(0.05)  # Wait a bit longer
    threading.Thread(target=network.send_message, args=("PC4", "Message from PC4", "PC1")).start()
